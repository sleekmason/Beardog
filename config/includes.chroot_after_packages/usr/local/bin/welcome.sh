...
--button=" Begin"!/usr/share/icons/Adwaita/22x22/legacy/preferences-desktop-font.png!:"x-terminal-emulator -T 'Customization' -e sudo xentry -i; bash" \
--button="gtk-ok:0" \
--text-info --justify=left --wrap < /usr/share/lilidog/welcome.txt --fontname="Dejavu Sans 11" \
--fore="#DBDBDB"; sed -i '/welcome.sh &/d' ~/.config/openbox/autostart && exit
else
  yad --title "Welcome!" --window-icon=/usr/share/icons/ld-icons/paw-color.png \
--width=375 --height=340 --center --escape-ok --undecorated --skip-taskbar \
--button="gtk-ok:0" \
--text-info --justify=left --wrap < /usr/share/lilidog/welcome2.txt --fontname="Dejavu Sans 11" \
